...
local colorscheme = loadfile("/etc/arch/colorscheme/additional.lua")() or {0xff0000,0x00ff00,0xffff00,0x0000ff,0xff00ff,0x00B6ff}
if #colorscheme ~= 6 or type(colorscheme) ~= table or
  type(colorscheme[1]) ~= number or type(colorscheme[2]) ~= number or type(colorscheme[3]) ~= number or
  type(colorscheme[4]) ~= number or type(colorscheme[5]) ~= number or type(colorscheme[6]) ~= number then
  colorscheme = {0xff0000,0x00ff00,0xffff00,0x0000ff,0xff00ff,0x00B6ff}
end

local _bg, _fg = 0x000000, 0xffffff
local bgf, fgf = fs.open("/etc/arch/colorscheme/bg"), fs.open("/etc/arch/colorscheme/fg")
bg, fg = tonumber(bgf:read(math.huge)), tonumber(fgf:read(math.huge))
bgf:close()
fgf:close()

table.insert(colorscheme, 1, bg)
table.insert(colorscheme, 8, fg)
